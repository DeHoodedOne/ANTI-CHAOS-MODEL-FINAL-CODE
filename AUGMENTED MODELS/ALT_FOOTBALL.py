...
def print_all_data():
    ...
    print(f"Countries: {len(countries)}, Leagues: {len(leagues)}, Home Teams: {len(home_teams)}, "
          ...
          f"Odds _1HWS: {len(_1HWS)}, Odds _2AWS: {len(_2AWS)}, "
          f" _12S: {len(_12S)}"
...
for i in range(len(match_timez)):
    try:
        divs[i].click()
    except (IndexError, StaleElementReferenceException, ElementClickInterceptedException, ElementClickInterceptedException):
        scheduler()
        try:
            divs = driver.find_elements(By.CSS_SELECTOR, '.soccer .event__match--scheduled')
            divs[i].click()
    ...
    print(f"{sport_alt}:- {i + 1}/{len(sub_m_t)}")
    sleep(0.1)
                    ...
                    for i in range(11):
                        with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_{i}.txt", mode="a") as file:
                            file.write(f"CONFIDENCE INTERVAL - {CONFINTVL}~COUNTRIES|LEAGUES|HOME TEAMS|AWAY TEAMS|HOME POSITIONS|AWAY POSITIONS|HOME NMP|AWAY NMP|"
                                       f"HOME PTS|AWAY PTS|HW FORM|HD FORM|HL FORM|AW FORM|AD FORM|AL FORM|HT_GAVG|AT_GAVG|BTH_GAVG|BTA_GAVG|BAYESIAN HOME WIN|"
                                       f"BAYESIAN AWAY WIN|BAYESIAN GG|BAYESIAN 1.5|BAYESIAN 2.5|BAYESIAN 12|MATCH TIME|SPORT|DIRECTORY|MATCH DATE\n")
                try:
                    if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                        if (home_nums_matches_played[q] >= NMP and away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                            if h_form_D[q] == 0 and a_form_D[q] == 0:
                                if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                    if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                        ...
                                        if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q]:
                                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_0.txt", mode="a") as file:
                                                writer(w=q, A=X, B=Y, C=Z, data=file)
                                ...
                                if h_form_D[q] == 0 and a_form_D[q] == 0:
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                            ...
                                            if abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2) and (0.5 * bayesian_12_d[q]) >= 0.5 * abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]):
                                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_0.txt", mode="a") as file:
                                                    writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
