...
for sport_ in SPORTS:
    ...
    def total_aggregator():
        ...
        try:
            ...
            if len(sub3) == 0:
                h2h_appender(prb_HW=0.01, prb_AW=0.1, prb_12=0.01)
                bth_g_avgs.append(0)
                bta_g_avgs.append(0)
            else:
                dsc = 0
    ...
    except (StaleElementReferenceException, ElementNotInteractableException, UnexpectedAlertPresentException):
        sub_m_t = []
        driver.refresh()
        sleep(2)
        scheduler()
    ...
    for g in range(len(match_timez)):
        try:
            divs[g].click()
        except (IndexError, StaleElementReferenceException, ElementNotInteractableException, ElementClickInterceptedException):
            scheduler()
            try:
                divs = driver.find_elements(By.CSS_SELECTOR, f'.{sport_alt} .event__match--scheduled')
                divs[g].click()
...
if is_running(game_criteria()[12][0]) is False and is_running(game_criteria()[12][2]) is False and is_running(game_criteria()[12][3]) is False:
    try:
        cleanse()
