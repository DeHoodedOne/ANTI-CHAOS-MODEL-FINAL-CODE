...
for sport_ in SPORTS:
    ...
    def h2h_aggregator(m, home_score, away_score, home_x, away_x):
        ...
        if (int(away_score[m].text) > int(home_score[m].text)) and (away_x[m].text == home_teamx):
            count_HW += 1
        if int(home_score[m].text) == int(away_score[m].text) and (
                int(home_score[m].text) > 0 and int(away_score[m].text) > 0):
            count_HW += 0.5
            count_AW += 0.5
        if int(home_score[m].text) == int(away_score[m].text) and (
                int(home_score[m].text) == 0 and int(away_score[m].text) == 0):
    ...
    def ht_indp_aggregator(k, home_score, away_score, home_x, away_x):
        ...
        if (int(away_score[k].text) > int(home_score[k].text)) and (away_x[k].text == home_teamx):
            count_HW_H_indp += 1
        if int(home_score[k].text) == int(away_score[k].text) and (
                int(home_score[k].text) > 0 and int(away_score[k].text) > 0):
    ...
    def at_indp_aggregator(n, home_score, away_score, home_x, away_x):
        ...
        if (int(away_score[n].text) > int(home_score[n].text)) and (away_x[n].text == away_teamx):
            count_AW_A_indp += 1
        if int(home_score[n].text) == int(away_score[n].text) and (
                int(home_score[n].text) > 0 and int(away_score[n].text) > 0):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    _12S.append(0.99)
                    print("No 1X2 ODDS")
            except (IndexError, ElementClickInterceptedException, ElementNotInteractableException):
                try:
                    driver.refresh()
                    sleep(2)
                    _1HW = driver.find_elements(By.CSS_SELECTOR, f'.oddsTab__tableWrapper .ui-table__row:nth-child(1) .oddsCell__odd')[0].text.strip()
    ...
    def writer(w, A, B, C, data):
        data.write(f"{countries[w]}|{leagues[w]}|{home_teams[w]}|{away_teams[w]}|{home_positions[w]}|"
                   ...
                   f"{OUTCOMES[2]}:-{bayesian_12_d[w]}|"
                   f"{match_times[w]}|{sport_alt}|{C}_{A}_{B}|{set_date_s_size()[0]}\n")
    ...
    def writer1(y, F, G, H, J, data):
        data.write(f"{countries[y]}|{leagues[y]}|{home_teams[y]}|{away_teams[y]}|{home_positions[y]}|"
                   ...
                   f"{OUTCOMES[2]}:-{bayesian_12_d[y]}|"
                   f"{match_times[y]}|{sport_alt}|{J}_{F}_{G}_{H}|{set_date_s_size()[0]}\n")
    ...
    def writer2(z, K, L, M, N, P, data):
        data.write(f"{countries[z]}|{leagues[z]}|{home_teams[z]}|{away_teams[z]}|{home_positions[z]}|"
                   ...
                   f"{OUTCOMES[2]}:-{bayesian_12_d[z]}|"
                   f"{match_times[z]}|{sport_alt}|{P}_{K}_{L}_{M}_{N}|{set_date_s_size()[0]}\n")
    ...
    def total_aggregator():
        ...
        try:
            ...
            if len(sub1) == 0:
                ht_indp_appender(prb_HW_H_indp=0.01, prb_AW_H_indp=0.01, prb_12_H_indp=0.99)
                ht_scores = []
        ...
        try:
            ...
            if len(sub2) == 0:
                at_indp_appender(prb_HW_A_indp=0.01, prb_AW_A_indp=0.01, prb_12_A_indp=0.99)
                at_scores = []
                for r in range(DSCRM_H_A):
        ...
        try:
            ...
            if len(sub3) == 0:
                h2h_appender(prb_HW=0.01, prb_AW=0.1, prb_12=0.99)
                bth_scores = []
                bta_scores = []
    ...
    def print_all_data():
        ...
        print(f"Prob HW_Z: {home_z_prob}")
        print(f"Prob AW_Z: {away_z_prob}")
        print(f"Prob DIFF_Z: {diff_z_prob}")
        ...
        print(f"Countries: {len(countries)}, Leagues: {len(leagues)}, Home Teams: {len(home_teams)}, "
              ...
              f"Prob HW_H_indps: {len(prob_HW_H_indps)}, Prob AW_H_indps: {len(prob_AW_H_indps)}, Prob 12_H_indps: {len(prob_12_H_indps)}, "
              f"Prob HW_A_indps: {len(prob_HW_A_indps)}, Prob AW_A_indps: {len(prob_AW_A_indps)}, Prob 12_A_indps: {len(prob_12_A_indps)}, "
              f"Prob HW_Z: {len(home_z_prob)}, Prob AW_Z: {len(away_z_prob)}, Prob DIFF_Z: {len(diff_z_prob)} "
    ...
    sport_alt = sport_.split('/')[3]
    divs = driver.find_elements(By.CSS_SELECTOR, f'.{sport_alt} .event__match--scheduled')
    print(len(divs))
    ...
    for g in range(len(match_timez)):
        try:
            divs[g].click()
        except (IndexError, StaleElementReferenceException, ElementClickInterceptedException, ElementNotInteractableException, UnexpectedAlertPresentException):
            scheduler()
            try:
                divs = driver.find_elements(By.CSS_SELECTOR, f'.{sport_alt} .event__match--scheduled')
                divs[g].click()
        ...
        print(f"{sport_alt}:- {g + 1}/{len(sub_m_t)}")
        sleep(0.1)
        ...
        try:
            ...
            try:
                pts_pos_mp()
                current_ht_pts = home_points[g]
                current_at_pts = away_points[g]
            ...
            count_12_H_indp = 0
            count_12_A_indp = 0
            for i in range(INDP_MATCH_SAMPLE):
                show_more_matches()
            ...
            try:
                ...
                team_data()
                odds_checker()
                H2H = driver.find_element(By.LINK_TEXT, "H2H")
                H2H.click()
                sleep(0.5)
                valid_games = 0
                count_HW = 0
                ...
                s_sports = ["tennis", "darts", "badminton", "snooker"]
                if sport_alt in s_sports:
                    current_ht_pos = home_positions[g]
                    current_at_pos = away_positions[g]
    ...
    for CONFINTVL in CONFINTVLS:
        for COMBINER in COMBINERS[:4]:
            ...
            for COMB in COMB_PROB_METRICS1:
                ...
                for q in range(len(bayesian_home_Win_d)):
                    if q == 0:
                        for i in range(11):
                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_{i}.txt", mode="a") as file:
                                file.write(f"CONFIDENCE INTERVAL - {CONFINTVL}~COUNTRIES|LEAGUES|HOME TEAMS|AWAY TEAMS|HOME POSITIONS|AWAY POSITIONS|HOME NMP|AWAY NMP|"
                                           f"HOME PTS|AWAY PTS|HW FORM|HD FORM|HL FORM|AW FORM|AD FORM|AL FORM|HT_GAVG|AT_GAVG|BTH_GAVG|BTA_GAVG|BAYESIAN HOME WIN|BAYESIAN AWAY WIN|"
                                           f"BAYESIAN 12|MATCH TIME|SPORT|DIRECTORY|MATCH DATE\n")
                    try:
                        if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                            if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                        ...
                                        if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_0.txt", mode="a") as file:
                                ...
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_1.txt", mode="a") as file:
                                                        writer(w=q, A=X, B=Y, C=Z, data=file)
                                        else:
                                            if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_1.txt", mode="a") as file:
                            ...
                            if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                if h_form_W[q] < a_form_W[q] and h_form_L[q] > a_form_L[q] and h_form_D[q] > a_form_D[q] and abs(h_form_W[q] - a_form_W[q]) > FORM_DIFF:
                                    if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                        if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                            if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_2.txt", mode="a") as file:
                                                    writer(w=q, A=X, B=Y, C=Z, data=file)
                                    else:
                                        if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_2.txt", mode="a") as file:
                            ...
                            if ((h_form_L[q] + h_form_D[q] >= FORM_VALUE and h_form_L[q] > h_form_D[q]) and (a_form_W[q] + a_form_D[q] >= FORM_VALUE and a_form_W[q] > a_form_D[q])) or ((a_form_L[q] + a_form_D[q] >= FORM_VALUE and a_form_L[q] > a_form_D[q]) and (h_form_W[q] + h_form_D[q] >= FORM_VALUE and h_form_W[q] > h_form_D[q])):
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                        if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                            if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_3.txt", mode="a") as file:
                                                    writer(w=q, A=X, B=Y, C=Z, data=file)
                                    else:
                                        if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_3.txt", mode="a") as file:
                                ...
                                if h_form_D[q] == 0 or a_form_D[q] == 0:
                                    if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                        if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_4.txt", mode="a") as file:
                                                        writer(w=q, A=X, B=Y, C=Z, data=file)
                                        else:
                                            if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_4.txt", mode="a") as file:
                            ...
                            if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                if h_form_W[q] > a_form_W[q] and h_form_L[q] < a_form_L[q] and h_form_D[q] < a_form_D[q] and abs(h_form_W[q] - a_form_W[q]) > FORM_DIFF:
                                    if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                        if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                            if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_5.txt", mode="a") as file:
                                                    writer(w=q, A=X, B=Y, C=Z, data=file)
                                    else:
                                        if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_5.txt", mode="a") as file:
                                ...
                                if bayesian_home_Win_d[q] > bayesian_away_Win_d[q] and ht_g_avgs[q] > at_g_avgs[q] and bth_g_avgs[q] > bta_g_avgs[q] and home_positions[q] < away_positions[q] and abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_6.txt", mode="a") as file:
                                                        writer(w=q, A=X, B=Y, C=Z, data=file)
                                        else:
                                            if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_6.txt", mode="a") as file:
                                ...
                                if abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_7.txt", mode="a") as file:
                                                        writer(w=q, A=X, B=Y, C=Z, data=file)
                                        else:
                                            if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_7.txt", mode="a") as file:
                                ...
                                if h_form_W[q] == 1.0 or a_form_W[q] == 1.0:
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_8.txt", mode="a") as file:
                                                        writer(w=q, A=X, B=Y, C=Z, data=file)
                                        else:
                                            if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_8.txt", mode="a") as file:
                                ...
                                if bayesian_home_Win_d[q] < bayesian_away_Win_d[q] and ht_g_avgs[q] < at_g_avgs[q] and bth_g_avgs[q] < bta_g_avgs[q] and home_positions[q] > away_positions[q] and abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_9.txt", mode="a") as file:
                                                        writer(w=q, A=X, B=Y, C=Z, data=file)
                                        else:
                                            if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_9.txt", mode="a") as file:
                        ...
                        if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                            if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                    if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                        if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport_alt}_alt_10.txt", mode="a") as file:
                                                writer(w=q, A=X, B=Y, C=Z, data=file)
                                    else:
            ...
            if Z != COMBINERS[2] and Z != COMBINERS[3] and Z != COMBINERS[6] and Z != COMBINERS[7]:
                for COMB in COMB_PROB_METRICS3:
                    ...
                    for q in range(len(bayesian_home_Win_d)):
                        if q == 0:
                            for i in range(11):
                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_{i}.txt", mode="a") as file:
                                    file.write(f"CONFIDENCE INTERVAL - {CONFINTVL}~COUNTRIES|LEAGUES|HOME TEAMS|AWAY TEAMS|HOME POSITIONS|AWAY POSITIONS|HOME NMP|AWAY NMP|"
                                               f"HOME PTS|AWAY PTS|HW FORM|HD FORM|HL FORM|AW FORM|AD FORM|AL FORM|HT_GAVG|AT_GAVG|BTH_GAVG|BTA_GAVG|BAYESIAN HOME WIN|BAYESIAN AWAY WIN|"
                                               f"BAYESIAN 12|MATCH TIME|SPORT|DIRECTORY|MATCH DATE\n")
                        try:
                                        ...
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                                if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                    if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                        with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_0.txt", mode="a") as file:
                                    ...
                                    if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                                if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                    if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                        with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_1.txt", mode="a") as file:
                                                            writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                            else:
                                                if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_1.txt", mode="a") as file:
                                ...
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if h_form_W[q] < a_form_W[q] and h_form_L[q] > a_form_L[q] and h_form_D[q] > a_form_D[q] and abs(h_form_W[q] - a_form_W[q]) > FORM_DIFF:
                                        if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_2.txt", mode="a") as file:
                                                        writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                        else:
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_2.txt", mode="a") as file:
                                ...
                                if ((h_form_L[q] + h_form_D[q] >= FORM_VALUE and h_form_L[q] > h_form_D[q]) and (a_form_W[q] + a_form_D[q] >= FORM_VALUE and a_form_W[q] > a_form_D[q])) or ((a_form_L[q] + a_form_D[q] >= FORM_VALUE and a_form_L[q] > a_form_D[q]) and (h_form_W[q] + h_form_D[q] >= FORM_VALUE and h_form_W[q] > h_form_D[q])):
                                    if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                        if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_3.txt", mode="a") as file:
                                                        writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                        else:
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_3.txt", mode="a") as file:
                                    ...
                                    if h_form_D[q] == 0 or a_form_D[q] == 0:
                                        if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                            if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                                if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                    if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                        with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_4.txt", mode="a") as file:
                                                            writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                            else:
                                                if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_4.txt", mode="a") as file:
                                ...
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if h_form_W[q] > a_form_W[q] and h_form_L[q] < a_form_L[q] and h_form_D[q] < a_form_D[q] and abs(h_form_W[q] - a_form_W[q]) > FORM_DIFF:
                                        if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                            if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_5.txt", mode="a") as file:
                                                        writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                        else:
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_5.txt", mode="a") as file:
                                    ...
                                    if bayesian_home_Win_d[q] > bayesian_away_Win_d[q] and ht_g_avgs[q] > at_g_avgs[q] and bth_g_avgs[q] > bta_g_avgs[q] and home_positions[q] < away_positions[q] and abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                                if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                    if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                        with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_6.txt", mode="a") as file:
                                                            writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                            else:
                                                if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_6.txt", mode="a") as file:
                                    ...
                                    if abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                                if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                    if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                        with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_7.txt", mode="a") as file:
                                                            writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                            else:
                                                if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_7.txt", mode="a") as file:
                                    ...
                                    if h_form_W[q] == 1.0 or a_form_W[q] == 1.0:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                                if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                    if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                        with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_8.txt", mode="a") as file:
                                                            writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                            else:
                                                if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_8.txt", mode="a") as file:
                                    ...
                                    if bayesian_home_Win_d[q] < bayesian_away_Win_d[q] and ht_g_avgs[q] < at_g_avgs[q] and bth_g_avgs[q] < bta_g_avgs[q] and home_positions[q] > away_positions[q] and abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                                if (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                    if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                        with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_9.txt", mode="a") as file:
                                                            writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                            else:
                                                if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_9.txt", mode="a") as file:
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport_alt}_alt_10.txt", mode="a") as file:
                                                    writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                        else:
            ...
            if Z != COMBINERS[2] and Z != COMBINERS[3] and Z != COMBINERS[4] and Z != COMBINERS[5]:
                for COMB in COMB_PROB_METRICS4:
                    ...
                    for q in range(len(bayesian_home_Win_d)):
                        if q == 0:
                            for i in range(11):
                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_{i}.txt", mode="a") as file:
                                    file.write(f"CONFIDENCE INTERVAL - {CONFINTVL}~COUNTRIES|LEAGUES|HOME TEAMS|AWAY TEAMS|HOME POSITIONS|AWAY POSITIONS|HOME NMP|AWAY NMP|"
                                               f"HOME PTS|AWAY PTS|HW FORM|HD FORM|HL FORM|AW FORM|AD FORM|AL FORM|HT_GAVG|AT_GAVG|BTH_GAVG|BTA_GAVG|BAYESIAN HOME WIN|BAYESIAN AWAY WIN|"
                                               f"BAYESIAN 12|MATCH TIME|SPORT|DIRECTORY|MATCH DATE\n")
                        try:
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if h_form_D[q] == 0 and a_form_D[q] == 0:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_0.txt", mode="a") as file:
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if ((home_positions[q] <= POS_MARK < away_positions[q]) or (away_positions[q] <= POS_MARK < home_positions[q])) and (home_positions[q] > 0 and away_positions[q] > 0):
                                    if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_1.txt", mode="a") as file:
                                                    writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if h_form_W[q] < a_form_W[q] and h_form_L[q] > a_form_L[q] and h_form_D[q] > a_form_D[q] and abs(h_form_W[q] - a_form_W[q]) > FORM_DIFF:
                                        if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_2.txt", mode="a") as file:
                                                writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if ((h_form_L[q] + h_form_D[q] >= FORM_VALUE and h_form_L[q] > h_form_D[q]) and (a_form_W[q] + a_form_D[q] >= FORM_VALUE and a_form_W[q] > a_form_D[q])) or ((a_form_L[q] + a_form_D[q] >= FORM_VALUE and a_form_L[q] > a_form_D[q]) and (h_form_W[q] + h_form_D[q] >= FORM_VALUE and h_form_W[q] > h_form_D[q])):
                                    if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                        if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_3.txt", mode="a") as file:
                                                writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                    if h_form_D[q] == 0 or a_form_D[q] == 0:
                                        if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_4.txt", mode="a") as file:
                                                    writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if h_form_W[q] > a_form_W[q] and h_form_L[q] < a_form_L[q] and h_form_D[q] < a_form_D[q] and abs(h_form_W[q] - a_form_W[q]) > FORM_DIFF:
                                        if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_5.txt", mode="a") as file:
                                                writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if bayesian_home_Win_d[q] > bayesian_away_Win_d[q] and ht_g_avgs[q] > at_g_avgs[q] and bth_g_avgs[q] > bta_g_avgs[q] and home_positions[q] < away_positions[q] and abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_6.txt", mode="a") as file:
                                                    writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_7.txt", mode="a") as file:
                                                    writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if h_form_W[q] == 1.0 or a_form_W[q] == 1.0:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_8.txt", mode="a") as file:
                                                    writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if bayesian_home_Win_d[q] < bayesian_away_Win_d[q] and ht_g_avgs[q] < at_g_avgs[q] and bth_g_avgs[q] < bta_g_avgs[q] and home_positions[q] > away_positions[q] and abs(home_positions[q] - away_positions[q]) >= POS_DIFF:
                                        if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                            if (BAY_DIFF / 2) <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= (0.5 * bayesian_12_d[q]) and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                                with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_9.txt", mode="a") as file:
                                                    writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
                            ...
                            if bayesian_home_Win_d[q] > CONFINTVL or bayesian_away_Win_d[q] > CONFINTVL:
                                if (home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP) and (((ht_g_avgs[q] > at_g_avgs[q]) and (bth_g_avgs[q] > bta_g_avgs[q])) or ((ht_g_avgs[q] < at_g_avgs[q]) and (bth_g_avgs[q] < bta_g_avgs[q]))):
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q] and bayesian_12_d[q] >= CONFINTVLS[-1]:
                                            with open(f"../RESULTS - {Z}_{V}_{W}_{X}_{Y}/{Z}_{V}_{W}_{X}_{Y}_{sport_alt}_alt_10.txt", mode="a") as file:
                                                writer2(z=q, K=V, L=W, M=X, N=Y, P=Z, data=file)
        ...
        if CONFINTVLS.index(CONFINTVL) == 0:
            with open(f"../LISTS - SPORTS DATA/SPORTS_DATA_LIST_{sport_alt}.txt", mode="a") as file:
                try:
...
if is_running(game_criteria()[12][0]) is False and is_running(game_criteria()[12][1]) is False and is_running(game_criteria()[12][2]) is False:
    try:
        cleanse()
