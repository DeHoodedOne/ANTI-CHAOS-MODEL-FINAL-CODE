...
OUTCOMES = ["HW", "AW", "GG", "ov1.5", "ov2.5", "12"]
    ...
    try:
        ...
        try:
            ...
            for z in range(len(ahl)):
                try:
                    id_1_5 = driver.find_element(By.CSS_SELECTOR, f'.oddsTab__tableWrapper .oddsCell__odds:nth-child({z + 1}) .ui-table__row:nth-child(1) .oddsCell__noOddsCell').text
                    if id_1_5 == "1.5":
                        OVER_1_5 = driver.find_elements(By.CSS_SELECTOR, f'.oddsTab__tableWrapper .oddsCell__odds:nth-child({z + 1}) .ui-table__row:nth-child(1) .oddsCell__odd')[0].text.strip()
...
def writer(w, A, B, C, data):
    data.write(f"{countries[w]}|{leagues[w]}|{home_teams[w]}|{away_teams[w]}|{home_positions[w]}|"
               ...
               f"{OUTCOMES[3]}:-{bayesian_1_5_d[w]}|{OUTCOMES[4]}:-{bayesian_2_5_d[w]}|"
               f"{OUTCOMES[5]}:-{bayesian_12_d[w]}|"
...
def writer1(y, F, G, H, J, data):
    data.write(f"{countries[y]}|{leagues[y]}|{home_teams[y]}|{away_teams[y]}|{home_positions[y]}|"
               ...
               f"{OUTCOMES[3]}:-{bayesian_1_5_d[y]}|{OUTCOMES[4]}:-{bayesian_2_5_d[y]}|"
               f"{OUTCOMES[5]}:-{bayesian_12_d[y]}|"
...
def writer2(z, K, L, M, N, P, data):
    data.write(f"{countries[z]}|{leagues[z]}|{home_teams[z]}|{away_teams[z]}|{home_positions[z]}|"
               ...
               f"{OUTCOMES[3]}:-{bayesian_1_5_d[z]}|{OUTCOMES[4]}:-{bayesian_2_5_d[z]}|"
               f"{OUTCOMES[5]}:-{bayesian_12_d[z]}|"
               f"{match_times[z]}|{sport}|{P}_{K}_{L}_{M}_{N}|{set_date_s_size()[0]}\n")


def baye_appender(baye_HW, baye_AW, baye_GG, baye_1_5, baye_2_5, baye_12):
        ...
        if len(sub3) == 0:
            ...
            if dsc >= DSCRM_BT:
                try:
                    ...
                    h2h_appender(prb_HW=h2h_values[0], prb_AW=h2h_values[1], prb_GG=h2h_values[2], prb_1_5=h2h_values[3], prb_2_5=h2h_values[4], prb_12=h2h_values[5])
                except ZeroDivisionError:
                    h2h_appender(prb_HW=0.01, prb_AW=0.1, prb_GG=0.01, prb_1_5=0.01, prb_2_5=0.01, prb_12=0.01)
            else:
                h2h_appender(prb_HW=0.01, prb_AW=0.1, prb_GG=0.01, prb_1_5=0.01, prb_2_5=0.01, prb_12=0.01)
            ...
            for q in range(len(home_teams)):
                try:
                    if X == PROB_METRICS[0] and Y == PROB_METRICS[1] and Z == COMBINERS[0]:
                        ...
                        baye_appender(baye_HW=bayesian_home_Win, baye_AW=bayesian_away_Win, baye_GG=bayesian_GG, baye_1_5=bayesian_1_5, baye_2_5=bayesian_2_5, baye_12=bayesian_12)
                            ...
                            if h_form_D[q] == 0 and a_form_D[q] == 0:
                                if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                    if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                        if bayesian_1_5_d[q] >= bayesian_2_5_d[q] and bayesian_1_5_d[q] >= bayesian_GG_d[q] and (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF:
                                        ...
                                        if bayesian_1_5_d[q] >= bayesian_2_5_d[q] and bayesian_1_5_d[q] >= bayesian_GG_d[q] and BAY_DIFF <= abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) <= bayesian_12_d[q]:
                                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport}_0.txt", mode="a") as file:
                                                writer(w=q, A=X, B=Y, C=Z, data=file)
                        ...
                        if home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP:
                            if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                if X == PROB_METRICS[1] or Y == PROB_METRICS[1]:
                                    ...
                                    if bayesian_1_5_d[q] >= bayesian_2_5_d[q] and bayesian_1_5_d[q] >= bayesian_GG_d[q] and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= BAY_DIFF and (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1:
                                        if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]):
                                            with open(f"../RESULTS - {Z}_{X}_{Y}/{Z}_{X}_{Y}_{sport}_10.txt", mode="a") as file:
                                                writer(w=q, A=X, B=Y, C=Z, data=file)
        ...
        for q in range(len(home_teams)):
            try:
                ...
                if X == PROB_METRICS1[0] and Y == PROB_METRICS1[2]:
                    ...
                    prob_1_5_indp_home = rdm.random()
                    prob_1_5_indp_away = rdm.random()
                    bayesian_1_5 = diff_z_prob[q] * (((math.sqrt(0.5 * (diff_z_prob[q] + prob_1_5s[q]))) * (prob_1_5_indp_home * prob_1_5_indp_away)) / (((math.sqrt(0.5 * (diff_z_prob[q] + prob_1_5s[q]))) * (prob_1_5_indp_home * prob_1_5_indp_away)) + ((1 - (prob_1_5_indp_home * prob_1_5_indp_away)) * (1 - (math.sqrt(0.5 * (diff_z_prob[q] + prob_1_5s[q])))))))
                    prob_2_5_indp_home = rdm.random()
                    prob_2_5_indp_away = rdm.random()
                                ...
                                if h_form_D[q] == 0 and a_form_D[q] == 0:
                                    if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                        if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                            if bayesian_1_5_d[q] >= bayesian_2_5_d[q] and bayesian_1_5_d[q] >= bayesian_GG_d[q] and (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1 and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2):
                                                if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]):
                                                    with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport}_0.txt", mode="a") as file:
                                                        writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                                        else:
                                            if bayesian_1_5_d[q] >= bayesian_2_5_d[q] and bayesian_1_5_d[q] >= bayesian_GG_d[q] and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2) and (0.5 * bayesian_12_d[q]) >= 0.5 * abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]):
                                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport}_0.txt", mode="a") as file:
                                                    writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
                            ...
                            if home_nums_matches_played[q] >= NMP or away_nums_matches_played[q] >= NMP:
                                if (h_form_W[q] >= FORM_VALUE and a_form_W[q] <= (1 - FORM_VALUE)) or (a_form_W[q] >= FORM_VALUE and h_form_W[q] <= (1 - FORM_VALUE)):
                                    if W != PROB_METRICS[2] and X != PROB_METRICS[2] and Y != PROB_METRICS[2]:
                                        ...
                                        if bayesian_1_5_d[q] >= bayesian_2_5_d[q] and bayesian_1_5_d[q] >= bayesian_GG_d[q] and abs(bayesian_home_Win_d[q] - bayesian_away_Win_d[q]) >= (BAY_DIFF / 2) and (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]) <= 1:
                                            if bayesian_12_d[q] >= (bayesian_home_Win_d[q] + bayesian_away_Win_d[q]):
                                                with open(f"../RESULTS - {Z}_{W}_{X}_{Y}/{Z}_{W}_{X}_{Y}_{sport}_10.txt", mode="a") as file:
                                                    writer1(y=q, F=W, G=X, H=Y, J=Z, data=file)
